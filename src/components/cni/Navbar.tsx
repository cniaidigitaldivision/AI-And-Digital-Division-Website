...
export function Navbar() {
  ...
  const renderDesktopDropdown = (label: string) => {
    ...
    const items = label === "Services" 
      ? SERVICES.map(s => ({ title: s.title, href: `/services/${slugify(s.title)}` }))
    ...
    return (
      ...
      >
        <button 
          className={cn(
            "flex items-center gap-1 relative text-[0.7rem] uppercase tracking-[0.2em] transition-colors hover:text-[var(--gold-bright)]",
    ...
    return (
      <div className="flex flex-col">
        <button 
          ...
          className="flex items-center justify-between py-2.5 text-[0.72rem] uppercase tracking-[0.2em] text-white w-full"
        >
          {label}
