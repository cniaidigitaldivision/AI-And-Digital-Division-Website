...
function renderAdminEmail(input: ContactEmailInput) {
  const field = (label: string, value: string, isEmail = false) => `
    <div style="padding:14px 18px;background:#0d2420;border-radius:10px;border:1px solid #1e4a42;margin-bottom:16px;">
      ...
      <p style="margin:0;font-size:15px;color:#ffffff;">
        ${
          isEmail
            ? `<a href="mailto:${escapeHtml(value)}" style="color:#7ab8b0;text-decoration:none;">${escapeHtml(value)}</a>`
            : value
            ? escapeHtml(value)
            : `<span style="color:#4a7a72;font-style:italic;">Not provided</span>`
        }
      </p>
    </div>
  `;
...
function renderUserConfirmationEmail(input: ContactEmailInput) {
  return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
export async function sendContactEmail(input: ContactEmailInput) {
  ...
  if (!apiKey) {
    ...
    throw new Error(
      "Email service is not configured (missing RESEND_API_KEY). Please set it in .env.",
    );
  }

  // ── Dev mode warning ──────────────────────────────────────────────────────
  // If still using the test sender, user confirmation will only work if
  // input.email matches your Resend account email. Log a clear warning.
  if (fromEmail === "onboarding@resend.dev") {
    console.warn(
      "[Resend] Using test sender onboarding@resend.dev — " +
      "user confirmation email will only deliver to your Resend account email. " +
      "Verify your domain at https://resend.com/domains to enable delivery to all users."
    );
  }
  ...
  const [adminResult, userResult] = await Promise.all([
    // Email 1 → Admin (full enquiry details)
    resend.emails.send({
    ...
    // Email 2 → User confirmation (thank you + summary + WhatsApp CTA)
    resend.emails.send({
  ...
  // Admin email failure = hard error (lead must not be lost)
  ...
  // User confirmation failure = soft warning only (don't block the enquiry)
  if (userResult.error) {
    console.warn(
      "User confirmation email failed (check domain verification):",
      userResult.error
    );
  }
}